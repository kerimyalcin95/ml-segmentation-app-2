...
- `dataset` contains example data used by the application, including source images, label images, prediction input images, prediction label images, label configurations, and other example files. The `model` directory is omitted from the GitHub repository because the model file exceeds GitHub's file size limit.
